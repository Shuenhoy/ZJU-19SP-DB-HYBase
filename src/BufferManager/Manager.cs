...
namespace HYBase.BufferManager
{
    ...
    class BufferManager
    {
        ...
        public byte[] GetPageData(FileStream file, int pageNum)
            => hashTable.TryGetValue((file, pageNum)).BiBind<byte[]>(Some: page => page.Value.page.data
                , None: () =>
                {
                    var node = InternalAlloc();
                    ReadPage(file, pageNum, node.Value.value.data);
        ...
        private void ReadPage(FileStream file, int pageNum, byte[] data)
        {
            int offset = pageNum * Page.SIZE + Page.SIZE;
            file.Seek(offset, SeekOrigin.Begin);
            file.Read(data, 0, Page.SIZE);
        }
