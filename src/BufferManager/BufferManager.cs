...
namespace HYBase.BufferManager
{
    ...
    struct BufferPageDesc
    {
        ...
        public FileStream fd;          // OS file descriptor of this page
    }
    class BufferManager
    {
        ...
        void UnpinPage(FileStream file, int pageNum)
            => hashTable.TryGetValue((file, pageNum)).BiBind<Either<ErrorCode, Unit>>(
                Some: slot =>
                    ...
                    ? (from x in Unlink(slot)
                       from y in LinkHead(slot)
                       select Unit.Default)
                    : Right<ErrorCode, Unit>(Unit.Default)
                ,
                None: () => Left<ErrorCode, Unit>(ErrorCode.PAGENOTINBUF)
            ).First();

        void FlushPages(FileStream file);                   // Flush pages for file

        // Force a page to the disk, but do not remove from the buffer pool
        void ForcePages(FileStream file, int pageNum);
        ...
        // Write a page
        void WritePage(FileStream file, int pageNum, Span<byte> source);
